	...
	stage('Code Analysis Stage') {
		...
		{	println "${env.SONAR_CONFIG_NAME}"
			...
			println "${env.BUILD_NUMBER}"
            		sh "pwd"
			sh "${SCANNER_HOME}/bin/sonar-scanner -Dsonar.projectKey=FinalProject -Dsonar.sources=. -Dsonar.projectVersion=${env.BUILD_NUMBER} -Dsonar.login=${env.SONAR_AUTH_TOKEN}"
        						}
			}
	}
